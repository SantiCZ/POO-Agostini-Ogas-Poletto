// subscriptionspage.cpp CORREGIDO
    ...
    : QFrame(parent)
{
    ...
    nameRow->setStyleSheet(
        "background: transparent;"
        );

    QHBoxLayout *nameRowL =
    ...
    rightW->setStyleSheet(
        "background: transparent;"
        );

    QVBoxLayout *rightL =
        new QVBoxLayout(rightW);
    ...
    actionsW->setStyleSheet(
        "background: transparent;"
        );

    QHBoxLayout *actL =
        new QHBoxLayout(actionsW);
...
void SubscriptionsPage::setupUI()
{
    ...
    headerW->setStyleSheet(
        "background: transparent;"
        );

    QHBoxLayout *headerL =
        new QHBoxLayout(headerW);

    headerL->setContentsMargins(
        0,0,0,0
        );
    ...
    titleW->setStyleSheet(
        "background: transparent;"
        );

    QVBoxLayout *titleL =
        new QVBoxLayout(titleW);

    titleL->setContentsMargins(
        0,0,0,0
        );
    ...
    QHBoxLayout *statsL =
        new QHBoxLayout(statsRow);

    statsL->setContentsMargins(
        0,0,0,0
        );
    ...
    connect(
        ...
        &QPushButton::clicked,
        this,
        &SubscriptionsPage::onAddClicked
        );
    ...
    m_subListLayout->setContentsMargins(
        0,0,0,0
        );
...
void SubscriptionsPage::addSubCard(
    const Suscripcion &s
    )
{
    auto *card =
    ...
    connect(
        card,
        &SubCard::deleteRequested,
        this,
        [this](int id)
        {
            if (
                DataManager::instance()
                    .removeSuscripcion(id)
                )
            {
                refreshData();
            }
