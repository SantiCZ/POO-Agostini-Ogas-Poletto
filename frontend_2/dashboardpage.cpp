    ...
    : QFrame(parent)
{
    ...
    titleL->setStyleSheet("color: #64748B; font-size: 11px; font-weight: 500; letter-spacing: 0.5px; background: transparent;");

    QLabel *valueL = new QLabel(value);
    valueL->setStyleSheet(QString("color: %1; font-size: 22px; font-weight: 700; background: transparent;").arg(accentColor));
...
// ─────────────────────────────────────────────────────────────
// SubRow (fila para suscripción activa en el dashboard)
// ─────────────────────────────────────────────────────────────
class SubRow : public QWidget {
public:
    SubRow(const Suscripcion &sub, QWidget *parent = nullptr)
        : QWidget(parent)
    {
        setFixedHeight(58);
        setStyleSheet("background: transparent;");
        ...
        for (auto it = icons.begin(); it != icons.end(); ++it) {
            if (sub.nombreServicio.toLower().contains(it.key())) {
                icon = it.value();
                break;
            }
        }
        ...
        montoL->setStyleSheet("color: #38BDF8; font-size: 13px; font-weight: 700; background: transparent;");
...
void DashboardPage::setupUI()
{
    ...
    dateLabel->setStyleSheet("color: #4ADE80; font-size: 12px; font-weight: 500; background-color: rgba(74,222,128,0.08); border: 1px solid rgba(74,222,128,0.2); border-radius: 20px; padding: 6px 14px;");
...
void DashboardPage::buildRecentActivity(QVBoxLayout *layout)
{
    ...
    verTodo->setStyleSheet(R"(
        QPushButton { background: transparent; color: #4ADE80; border: none; font-size: 12px; font-weight: 600; }
    ...
    std::sort(tickets.begin(), tickets.end(),
              [](const Ticket& a, const Ticket& b) { return a.fecha > b.fecha; });
...
// ─────────────────────────────────────────────────────────────
// buildSubsAlert: muestra las suscripciones activas
// ─────────────────────────────────────────────────────────────
void DashboardPage::buildSubsAlert(QVBoxLayout *layout)
{
    ...
    QLabel *title = new QLabel("Suscripciones Activas");
    title->setStyleSheet("color: #F1F5F9; font-size: 16px; font-weight: 700; background: transparent;");
    ...
    connect(verTodo, &QPushButton::clicked, this, &DashboardPage::navigateToSubs);
    ...
    std::sort(activas.begin(), activas.end(),
              [](const Suscripcion &a, const Suscripcion &b) {
                  return a.fechaVencimiento < b.fechaVencimiento;
              });

    int maxItems = qMin(5, activas.size());
    for (int i = 0; i < maxItems; ++i) {
        SubRow *row = new SubRow(activas[i], this);
        cardL->addWidget(row);
        if (i < maxItems - 1) {
            QFrame *sep = new QFrame();
            sep->setFrameShape(QFrame::HLine);
    ...
    if (activas.isEmpty()) {
        QLabel *empty = new QLabel("No hay suscripciones activas.\nAgregá una desde el menú.");
        empty->setAlignment(Qt::AlignCenter);
        empty->setStyleSheet("color: #64748B; padding: 20px;");
        cardL->addWidget(empty);
    }
